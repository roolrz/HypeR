...
fn run(
    ...
    supervise_guest(&machine, &vcpu, control)
}
...
fn supervise_guest(
    ...
    let observation =
        wait_many(&waits, hyper_os::DEADLINE_INFINITE).map_err(Error::OperatingSystem)?;
        ...
        && ObjectSignals::<ByteChannelObject>::READABLE.is_present_in(observation.observed)
    {
        ...
        let vm_contract::InstanceCommand::Stop = command;
