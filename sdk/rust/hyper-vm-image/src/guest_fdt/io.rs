...
impl IoDevices<'_> {
    ...
    pub(super) fn validate(self, boot: Aarch64LinuxBoot<'_>) -> Result<(), Error> {
        ...
        let slots = [self.virtio, self.mailbox, self.notification];
        ...
        if self.virtio.is_none() || self.dma_ranges.len() > MAX_DMA_RANGES {
            return Err(Error::InvalidInput);
        }
    ...
    pub(super) fn append(
        ...
        if let Some(virtio) = self.virtio {
            ...
            node(builder, "virtio_mmio@", "virtio,mmio", virtio)?;
            if physical {
                builder.end_node()?;
            }
        }
