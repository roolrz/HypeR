...
| 75 | `virtual_serial_read` | `virtual_serial: handle`, `bytes: user_address`, `byte_capacity: byte_count` | `actual_bytes: byte_count` | `virtual_serial: Borrow, kind=virtual_serial, rights=0x10` | `bytes: Write, len=byte_capacity bytes, max-bytes=4096; order=0` | `blocking=MayBlock, cancellation=None, restart=Never, completion=Returns, flags=None` | `Capability` |
| 76 | `virtual_serial_write` | `virtual_serial: handle`, `bytes: user_address`, `byte_count: byte_count` | `actual_bytes: byte_count` | `virtual_serial: Borrow, kind=virtual_serial, rights=0x20` | `bytes: Read, len=byte_count bytes, max-bytes=4096; order=0` | `blocking=MayBlock, cancellation=None, restart=Never, completion=Returns, flags=None` | `Capability` |
