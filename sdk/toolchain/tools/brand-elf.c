...
static int validate_link_mode(FILE *file, const char *path, uint64_t file_size,
			      uint64_t entry_point, enum executable_link_mode link_mode,
			      int interpreter_headers, int dynamic_headers, uint64_t dynamic_offset,
			      uint64_t dynamic_size)
{
	...
	if (dynamic_headers != 1 || dynamic_size == 0 ||
	    dynamic_size % ELF_DYNAMIC_ENTRY_SIZE != 0 || dynamic_offset > file_size ||
...
static int validate_program_headers(FILE *file, const char *path, uint64_t file_size,
				    uint64_t offset, uint16_t entry_size, uint16_t count,
				    uint64_t entry_point, enum executable_link_mode link_mode)
{
	...
	for (uint16_t index = 0; index < count; ++index) {
		...
		if (entry_offset > (uint64_t)LONG_MAX || entry_offset > file_size ||
		    file_size - entry_offset < sizeof(header) ||
		    fseek(file, (long)entry_offset, SEEK_SET) != 0 ||
		...
		if (type == ELF_PROGRAM_GNU_STACK) {
			if (++stack_policy_headers != 1) {
				return fail(path,
					    "multiple stack-policy headers are not permitted");
			}
		...
		if ((alignment > 1 &&
		     (alignment > ELF_PAGE_SIZE || (alignment & (alignment - 1)) != 0 ||
		...
		if (memory_size != 0) {
			const uint64_t mapping_start =
				virtual_address & ~(uint64_t)(ELF_PAGE_SIZE - 1);
			...
			ranges[range_count++] = (struct load_range){
				.start = mapping_start,
				.end = (memory_end + ELF_PAGE_SIZE - 1) &
				       ~(uint64_t)(ELF_PAGE_SIZE - 1),
			};
		}
	...
	if (validate_link_mode(file, path, file_size, entry_point, link_mode, interpreter_headers,
			       dynamic_headers, dynamic_offset, dynamic_size) != EXIT_SUCCESS) {
		return EXIT_FAILURE;
	}
...
static int brand(const char *path, int check_only, enum executable_link_mode link_mode)
{
	...
	if (validate_program_headers(file, path, file_size, read_u64(header + 32),
				     read_u16(header + 54), read_u16(header + 56),
				     read_u64(header + 24), link_mode) != EXIT_SUCCESS) {
		fclose(file);
		return EXIT_FAILURE;
	}
	if (check_only) {
...
int main(int argc, char **argv)
{
	...
	{
		fputs("usage: hyper-brand-elf [--check|--check-static|--check-dynamic] IMAGE\n",
		      stderr);
		return EXIT_FAILURE;
	}
}
