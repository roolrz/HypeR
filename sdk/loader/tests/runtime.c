...
static void reset(void)
{
    ...
    segments[0] = (Elf64_Phdr){ .type = PT_LOAD, .flags = PF_R | PF_W,
        .memsz = sizeof(image) };
    segments[1] = (Elf64_Phdr){ .type = PT_DYNAMIC,
        .vaddr = offsetof(Image, dynamic), .memsz = 5 * sizeof(Elf64_Dyn) };
...
static void relocations(void)
{
    ...
    reset();
    uint64_t relr[] = { offsetof(Image, targets), 1 | (UINT64_C(1) << 1)
        | (UINT64_C(1) << 63), 3 };
    ...
    for (size_t i = 0; i <= MAX_RELOCATIONS; ++i) many[i] = (uintptr_t)&image.targets[0];
...
static void file_bounds(void)
{
    ...
    file_header = (Elf64_Ehdr){ .ident = { 0x7f, 'E', 'L', 'F', 2, 1, 1,
        HYPER_NATIVE_ELF_OSABI, HYPER_NATIVE_ELF_ABI_VERSION }, .type = ET_DYN,
        .machine = HYPER_ELF_MACHINE, .version = 1, .ehsize = sizeof(Elf64_Ehdr),
        .phentsize = sizeof(Elf64_Phdr), .phnum = 1, .phoff = sizeof(Elf64_Ehdr) };
#if defined(TEST_RISCV)
    file_header.flags = 4;
#endif
    ...
    file_segments[0] = (Elf64_Phdr){ .type = PT_LOAD, .flags = PF_R,
        .filesz = PAGE_SIZE, .memsz = PAGE_SIZE, .align = PAGE_SIZE };
    file_segments[1] = (Elf64_Phdr){ .type = PT_DYNAMIC, .vaddr = 512,
...
hyper_call_result_t hyper_native_call6(uint64_t n, uint64_t a, uint64_t b,
    uint64_t c, uint64_t d, uint64_t e, uint64_t f)
{ (void)n; (void)a; (void)b; (void)c; (void)d; (void)e; (void)f; abort(); }
hyper_call_result_t hyper_console_write(hyper_native_handle_t h, const void *b, size_t n)
{ (void)h; (void)b; (void)n; abort(); }
_Noreturn void hyper_process_exit(int64_t status) { (void)status; abort(); }
hyper_call_result_t hyper_directory_open_file(hyper_native_handle_t h,
    const void *p, size_t n, uint64_t r)
{ (void)h; (void)p; (void)n; (void)r; assert(allow_open); return (hyper_call_result_t){0, 20, 0}; }
hyper_call_result_t hyper_file_create_executable_vmo(hyper_native_handle_t h)
{ assert(h == 20); return (hyper_call_result_t){0, 21, file_length}; }
hyper_native_status_t hyper_handle_close(hyper_native_handle_t h)
{ effect(300 + (unsigned)h); return HYPER_NATIVE_STATUS_OK; }
hyper_native_status_t hyper_vmo_read(hyper_native_handle_t h, uint64_t off, void *out, size_t n)
{
    assert(h == 21); reads++;
    if (off == 0 && n == sizeof(file_header)) memcpy(out, &file_header, n);
    else if (off == sizeof(file_header) && n == sizeof(file_segments)) memcpy(out, file_segments, n);
    else abort();
...
{ assert(fail_mapping && h == root_vmar && a == LIBRARY_BASE && n == PAGE_SIZE);
    effect(400); return (hyper_call_result_t){0, 42, 0}; }
hyper_native_status_t hyper_vmar_map_private(hyper_native_handle_t h,
    hyper_native_handle_t s, const hyper_native_private_mapping_t *m, size_t n)
{ assert(fail_mapping && h == 42 && s == 21 && n == sizeof(*m));
    effect(500); return HYPER_NATIVE_STATUS_NO_MEMORY; }
hyper_native_status_t hyper_vmar_protect(hyper_native_handle_t h, uintptr_t a, size_t n, uint32_t p)
{ (void)h; (void)a; (void)n; (void)p; abort(); }
hyper_native_status_t hyper_vmar_unmap(hyper_native_handle_t h, uintptr_t a, size_t n)
{ assert(a >= LIBRARY_BASE && n == PAGE_SIZE); effect(100 + (unsigned)h); return 0; }
hyper_native_status_t hyper_vmar_destroy(hyper_native_handle_t h)
{ effect(200 + (unsigned)h); return h == 12 ? HYPER_NATIVE_STATUS_BAD_STATE : 0; }
