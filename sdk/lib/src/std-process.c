...
{
	if (!source)
		return HYPER_NATIVE_STATUS_ACCESS_DENIED;
	hyper_native_handle_info_t info = {0};
	int64_t status = hyper_handle_get_info(source, &info).status;
	if (status != 0)
		return status;
	return hyper_process_builder_add_handle(builder, source, purpose, info.object_kind,
						info.rights & ceiling,
						HYPER_NATIVE_CAPABILITY_DISPOSITION_DUPLICATE);
}

int64_t __hyper_std_process_begin(const char *program, size_t size, const char *cwd,
				  size_t cwd_size, uint64_t *output)
{
	*output = 0;
	...
	if (cwd_size) {
		...
		if (status)
			goto fail;
		hyper_call_result_t opened =
			hyper_directory_open_directory(base, cwd, cwd_size, info.rights);
		if (opened.status) {
	...
	uint64_t factory =
		hyper_runtime_capability(HYPER_NATIVE_STARTUP_HANDLE_PURPOSE_TASK_FACTORY);
	uint64_t group = hyper_runtime_capability(HYPER_NATIVE_STARTUP_HANDLE_PURPOSE_TASK_GROUP);
	uint64_t domain =
		hyper_runtime_capability(HYPER_NATIVE_STARTUP_HANDLE_PURPOSE_RESOURCE_DOMAIN);
	if (!base || !factory || !group || !domain) {
		status = HYPER_NATIVE_STATUS_ACCESS_DENIED;
		goto fail;
	}
	hyper_call_result_t file =
		hyper_directory_open_file(base, program, size, HYPER_NATIVE_RIGHT_EXECUTE);
	if (file.status) {
		status = file.status;
		goto fail;
	}
	hyper_call_result_t created =
		hyper_process_builder_create(factory, group, domain, file.value0);
	...
	if (name_size > HYPER_NATIVE_PROCESS_NAME_MAX_BYTES) {
		...
		while (name_size && ((unsigned char)program[name_start + name_size] & 0xc0) == 0x80)
			--name_size;
	}
	...
	if (!library)
		library = hyper_runtime_capability(
			HYPER_NATIVE_STARTUP_HANDLE_PURPOSE_DYNAMIC_LIBRARY_DIRECTORY);
	if (library) {
		status = delegate(builder, library,
				  HYPER_NATIVE_STARTUP_HANDLE_PURPOSE_DYNAMIC_LIBRARY_DIRECTORY,
				  UINT64_MAX);
		if (status)
			goto fail;
	...
	for (size_t i = 0; i < sizeof(inherited) / sizeof(inherited[0]); ++i) {
		uint64_t source = inherited[i] == HYPER_NATIVE_STARTUP_HANDLE_PURPOSE_ROOT_DIRECTORY
					  ? root
					  : hyper_runtime_capability(inherited[i]);
		if (!source)
			continue;
...
int64_t __hyper_std_process_pipe(uint64_t builder, uint32_t stream, uint64_t *parent)
{
	*parent = 0;
	...
	if (pair.status != 0)
		return pair.status;
	uint64_t rights = HYPER_NATIVE_RIGHT_WAIT | HYPER_NATIVE_RIGHT_DUPLICATE |
			  HYPER_NATIVE_RIGHT_TRANSFER |
			  (stream == 0 ? HYPER_NATIVE_RIGHT_READ : HYPER_NATIVE_RIGHT_WRITE);
	int64_t status = hyper_process_builder_add_handle(builder, pair.value1, STDIN + stream,
							  HYPER_NATIVE_OBJECT_BYTE_CHANNEL, rights,
							  HYPER_NATIVE_CAPABILITY_DISPOSITION_MOVE);
	if (status != 0) {
		(void)hyper_handle_close(pair.value0);
		(void)hyper_handle_close(pair.value1);
		return status;
	}
	*parent = pair.value0;
	return 0;
				    ...
				    uint32_t parent_stream)
{
	...
	hyper_native_handle_t alias = (!source && stream == 0 && parent_stream == 0)
					      ? hyper_runtime_capability(TERMINAL_INPUT)
					      : 0;
	if (!source)
		source = hyper_runtime_capability(STDIN + parent_stream);
	uint64_t rights = HYPER_NATIVE_RIGHT_DUPLICATE | HYPER_NATIVE_RIGHT_TRANSFER |
			  HYPER_NATIVE_RIGHT_WAIT |
			  (stream == 0 ? HYPER_NATIVE_RIGHT_READ : HYPER_NATIVE_RIGHT_WRITE);
	if (alias) {
...
int64_t __hyper_std_process_kill(uint64_t process)
{
	return hyper_process_request_stop(process);
}

int64_t __hyper_std_process_wait(uint64_t process, uint32_t block, uint32_t *done, uint32_t *reason,
				 int64_t *code)
{
	*done = 0;
	hyper_call_result_t wait =
		hyper_object_wait_one(process, HYPER_NATIVE_SIGNAL_PROCESS_TERMINATED,
				      block ? HYPER_NATIVE_DEADLINE_INFINITE : 0);
	if (wait.status == HYPER_NATIVE_STATUS_TIMED_OUT && !block)
		return 0;
	if (wait.status != 0)
		return wait.status;
...
int64_t __hyper_std_pipe_try_read(uint64_t handle, void *buffer, size_t capacity, size_t *actual)
{
	*actual = 0;
	...
	if (result.status != 0)
		return result.status;
	if (result.value0 == 0)
		return HYPER_NATIVE_STATUS_WOULD_BLOCK;
	...
	*actual = 0;
	for (;;) {
		...
		if (result.status != HYPER_NATIVE_STATUS_WOULD_BLOCK)
			return result.status;
		result = hyper_object_wait_one(handle,
...
int64_t __hyper_std_pipe_write(uint64_t handle, const void *buffer, size_t size, size_t *actual)
{
	*actual = 0;
	if (!size)
		return 0;
	if (size > HYPER_NATIVE_BYTE_CHANNEL_MAX_MESSAGE_BYTES)
		size = HYPER_NATIVE_BYTE_CHANNEL_MAX_MESSAGE_BYTES;
	for (;;) {
...
int64_t __hyper_std_wait_pair(uint64_t first, uint64_t second, uint64_t *set, uint64_t *first_id,
			      uint64_t *second_id)
{
	*set = 0;
	...
	const uint64_t mask = HYPER_NATIVE_SIGNAL_BYTE_CHANNEL_READABLE |
			      HYPER_NATIVE_SIGNAL_BYTE_CHANNEL_PEER_CLOSED;
	hyper_call_result_t a = hyper_wait_set_add(created.value0, first, mask);
	hyper_call_result_t b =
		a.status == 0 ? hyper_wait_set_add(created.value0, second, mask) : a;
	if (b.status != 0) {
		(void)hyper_handle_close(created.value0);
		return b.status;
	}
	*set = created.value0;
	*first_id = a.value0;
...
int64_t __hyper_std_wait_ready(uint64_t set, uint64_t *id)
{
	...
	hyper_call_result_t result =
		hyper_wait_set_wait(set, HYPER_NATIVE_DEADLINE_INFINITE, event, sizeof(event));
	if (result.status == 0)
		*id = event[0];
	return result.status;
}

int64_t __hyper_std_wait_rearm(uint64_t set, uint64_t id)
{
	return hyper_wait_set_rearm(set, id).status;
}

int64_t __hyper_std_wait_remove(uint64_t set, uint64_t id)
{
	return hyper_wait_set_remove(set, id).status;
}
