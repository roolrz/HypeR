...
hyper_native_status_t hyper_runtime_initialize(const uintptr_t *initial_stack)
{
    ...
    for (size_t i = 0; i < startup.handle_count; ++i) {
        if (startup.handles[i].purpose != HYPER_NATIVE_STARTUP_HANDLE_PURPOSE_INITIAL_STACK_VMAR)
            application_handles[count++] = startup.handles[i];
    }
    hyper_auxiliary_entry_t *auxiliary = calloc(startup.auxiliary_count + 1, sizeof(*auxiliary));
    if (!auxiliary) return HYPER_NATIVE_STATUS_NO_MEMORY;
