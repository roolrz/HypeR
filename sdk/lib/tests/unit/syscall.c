...
#define STATUS(expression) do { assert((expression) == status); check_consumed(); } while (0)
...
static void transport(hyper_native_status_t status)
{
    ...
    EXPECT(ABI_QUERY, 0); RESULT(hyper_abi_query());
    EXPECT(CLOCK_GET_MONOTONIC, 0); RESULT(hyper_clock_get_monotonic());
    EXPECT(CLOCK_GET_REALTIME, 0); RESULT(hyper_clock_get_realtime());
    EXPECT(HANDLE_CLOSE, handle); STATUS(hyper_handle_close(handle));
    EXPECT(HANDLE_DUPLICATE, handle, other); RESULT(hyper_handle_duplicate(handle, other));
    ...
    EXPECT(CAPABILITY_CHANNEL_TRY_SEND, handle, 0, (uintptr_t)bytes, sizeof(bytes), (uintptr_t)dispositions, 3);
    STATUS(hyper_capability_channel_try_send(handle, bytes, sizeof(bytes), dispositions, 3));
    EXPECT(CAPABILITY_CHANNEL_RECEIVE, handle, deadline, (uintptr_t)bytes, sizeof(bytes), (uintptr_t)slots, 2);
    ...
    EXPECT(DIRECTORY_RENAME, handle, (uintptr_t)bytes, sizeof(bytes), other, (uintptr_t)destination, sizeof(destination));
    RESULT(hyper_directory_rename(handle, bytes, sizeof(bytes), other, destination, sizeof(destination)));
    EXPECT(DIRECTORY_OPEN_FILE_WITH_OPTIONS, handle, (uintptr_t)bytes, sizeof(bytes), other, options, 0754);
