...
struct hyper_stack { unsigned cookie; };
...
static void check_original(const hyper_startup_t *startup)
{
    ...
    assert(hyper_startup_find_handle(startup,
        HYPER_NATIVE_STARTUP_HANDLE_PURPOSE_INITIAL_STACK_VMAR, &handle) == HYPER_NATIVE_STATUS_OK);
    assert(handle == 22);
}
...
int main(void)
{
    ...
    uintptr_t words[] = {
        2, (uintptr_t)argument, (uintptr_t)option, 0, (uintptr_t)environment, 0,
        HYPER_NATIVE_AUXV_STARTUP_HANDLES, (uintptr_t)handles,
        HYPER_NATIVE_AUXV_STARTUP_HANDLE_COUNT, 3,
        HYPER_NATIVE_AUXV_INITIAL_STACK_BASE, 0xf1000000,
        HYPER_NATIVE_AUXV_INITIAL_STACK_CAPACITY, 0x800000,
        HYPER_NATIVE_AUXV_INITIAL_STACK_SIZE, 0x40000,
        6, HYPER_NATIVE_PAGE_SIZE,
        0, 0,
    };
    ...
    assert(view->argument_count == 2 && view->arguments[0] == argument && view->arguments[1] == option);
    ...
    assert(hyper_startup_find_handle(view,
        HYPER_NATIVE_STARTUP_HANDLE_PURPOSE_INITIAL_STACK_VMAR, &output) != HYPER_NATIVE_STATUS_OK);
    assert(hyper_runtime_initialize(words) == HYPER_NATIVE_STATUS_OK);
