...
impl<'a> GuestSyncFrame<'a> {
    ...
    pub fn translation_fault(&self) -> Option<GuestTranslationFault> {
        let access = match self.trap.scause {
            CAUSE_INSTRUCTION_GUEST_PAGE_FAULT => GuestMemoryAccess::Execute,
            CAUSE_LOAD_GUEST_PAGE_FAULT => GuestMemoryAccess::Read,
            CAUSE_STORE_GUEST_PAGE_FAULT => GuestMemoryAccess::Write,
            _ => return None,
        };
        Some(GuestTranslationFault {
            address: (self.trap.htval << 2) | (self.trap.stval & 3),
            access,
            during_page_walk: self.trap.htinst != 0,
        })
    }
}
...
fn write_vsie(value: u64) {
    unsafe { asm!("csrw vsie, {value}", value = in(reg) value, options(nomem, nostack)) };
}
...
fn write_vsip(value: u64) {
    unsafe { asm!("csrw vsip, {value}", value = in(reg) value, options(nomem, nostack)) };
}

fn write_hcounteren(value: u64) {
    unsafe { asm!("csrw hcounteren, {value}", value = in(reg) value, options(nomem, nostack)) };
}
