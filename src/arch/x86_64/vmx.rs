...
fn handle_ept_violation() {
    ...
    let access = match violation.access {
        EptAccess::Read => super::GuestMemoryAccess::Read,
        EptAccess::Write => super::GuestMemoryAccess::Write,
        EptAccess::Execute => super::GuestMemoryAccess::Execute,
    };
...
fn timer_pending() -> &'static AtomicBool {
    TIMER_PENDING
        .get(super::current_cpu_index())
        .unwrap_or_else(|| crate::kernel::boot::fail("VMX timer CPU lookup", Error::InvalidCpu))
}
...
fn emulate_io(registers: &mut ExitRegisters) {
    ...
    let result =
        match crate::kernel::vm::handle_port_io(exit.port, exit.size, !input, registers.rax as u32)
        {
            Ok(result) => result,
            Err(error) => crate::kernel::boot::fail("legacy PC port I/O", error),
        };
    if let Some(value) = result {
        registers.rax = merge_port_input(registers.rax, value, exit.size)
            .unwrap_or_else(|| crate::kernel::boot::fail("invalid VMX I/O access size", exit.size));
    }
}
