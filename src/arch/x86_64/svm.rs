...
use hyper::vm::device::x86_legacy::InterruptSource;
...
struct SharedIoPermissionMap(UnsafeCell<IoPermissionMap>);
unsafe impl Sync for SharedIoPermissionMap {}

#[repr(C, align(4096))]
struct MsrPermissionMap([u8; PAGE_SIZE * 2]);
struct SharedMsrPermissionMap(UnsafeCell<MsrPermissionMap>);
unsafe impl Sync for SharedMsrPermissionMap {}
...
static IOPM: SharedIoPermissionMap =
    SharedIoPermissionMap(UnsafeCell::new(IoPermissionMap([0xff; PAGE_SIZE * 3])));
static MSRPM: SharedMsrPermissionMap =
    SharedMsrPermissionMap(UnsafeCell::new(MsrPermissionMap([0xff; PAGE_SIZE * 2])));
...
pub(super) unsafe fn enter(context: &mut VcpuContext) -> ! {
    ...
    let (vmcb, host_save) = match prepare(cpu, context) {
...
fn write_control_area(vmcb: &mut Page) -> Result<(), Error> {
    ...
    let iopm = unsafe { &*IOPM.0.get() };
    let msrpm = unsafe { &*MSRPM.0.get() };
...
fn emulate_io(vmcb: &Page, context: &mut VcpuContext) {
    ...
    let result = match crate::kernel::vm::handle_port_io(
        exit.port,
        exit.size,
        !input,
    ...
    if let Some(value) = result {
        ...
        active_vmcb().write_u64(SAVE_RAX, context.general[0]);
    }
}
...
fn handle_npf(vmcb: &Page) {
    ...
    let access = match violation.access {
        NptAccess::Read => super::GuestMemoryAccess::Read,
        NptAccess::Write => super::GuestMemoryAccess::Write,
        NptAccess::Execute => super::GuestMemoryAccess::Execute,
    };
    let mut frame = super::GuestSyncFrame::translation(super::GuestTranslationFault {
        address,
        access,
        during_page_walk: violation.during_page_walk,
    });
...
fn prepare_guest_interrupt(vmcb: &mut Page) {
    ...
    let pending =
        match crate::kernel::vm::legacy_pending_interrupt(timer_pending.load(Ordering::Acquire)) {
...
fn active_vmcb() -> &'static mut Page {
    let cpu = super::current_cpu_index();
    let vmcbs = unsafe { &mut *VMCBS.0.get() };
    vmcbs
        .get_mut(cpu)
        .unwrap_or_else(|| crate::kernel::boot::fail("SVM VMCB CPU lookup", Error::InvalidCpu))
}
