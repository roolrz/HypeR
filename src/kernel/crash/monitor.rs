...
use super::{CPU_CONTEXTS, MAX_CPUS, StopResult, dump_backtrace, dump_registers};
...
pub(super) fn run(owner: usize, stop: StopResult) {
    write_raw(b"\nHypeR crash console\n");
...
fn execute(line: &str, owner: usize, stop: StopResult) -> Action {
...
fn print_status(owner: usize, stop: StopResult) {
...
fn dump_cpu_registers(cpu: usize) {
    let Some(context) = CPU_CONTEXTS.get(cpu).and_then(|slot| slot.read()) else {
...
fn inspect_mapping_argument<'a>(arguments: &mut impl Iterator<Item = &'a str>) {
    ...
    match crate::arch::inspect_stage1_mapping(snapshot.root, address) {
...
fn validate_read(
    ...
    while cursor < end {
        let mapping = crate::arch::inspect_stage1_mapping(snapshot.root, cursor)
...
fn run_self_test(owner: usize, stop: StopResult) {
    ...
    let owner_context = CPU_CONTEXTS.get(owner).and_then(|slot| slot.read());
    report_test(
        "owner crash context",
    ...
    let owner_pc_is_mapped = match (snapshot, owner_context) {
        (Some(snapshot), Some(context)) => usize::try_from(context.program_counter)
            .ok()
            .and_then(|address| crate::arch::inspect_stage1_mapping(snapshot.root, address).ok())
            .flatten()
            .is_some(),
        _ => false,
    };
