...
pub fn handles(address: u64) -> bool {
    in_range(
        address,
        u64::from(DISTRIBUTOR_BASE),
        u64::from(DISTRIBUTOR_SIZE),
    ) || in_range(
        address,
        u64::from(REDISTRIBUTOR_BASE),
        u64::from(REDISTRIBUTOR_SIZE),
    )
}
...
pub fn read(
    ...
    if in_range(
        ...
        return read_distributor(interrupts, address - u64::from(DISTRIBUTOR_BASE), size);
    }
    if in_range(
        address,
        u64::from(REDISTRIBUTOR_BASE),
        ...
        return read_redistributor(
            interrupts,
            vcpu,
            address - u64::from(REDISTRIBUTOR_BASE),
            size,
        );
    }
...
pub fn write(
    ...
    if in_range(
        ...
        return write_distributor(
            interrupts,
            address - u64::from(DISTRIBUTOR_BASE),
            size,
            value,
        );
    }
