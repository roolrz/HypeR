...
pub(super) struct Scheduler {
    ...
    cpu_slots: [Option<usize>; MAX_CPUS],
    next_id: u64,
...
pub(super) struct SwitchPair {
    pub previous: *mut crate::arch::ThreadContext,
    pub next: *const crate::arch::ThreadContext,
}

impl CpuScheduler {
    const fn new(index: usize, current: ThreadId) -> Self {
        Self {
            index,
            current,
            idle: None,
...
impl Scheduler {
    pub fn new(boot_cpu: usize) -> Result<Self, Error> {
        ...
        let mut cpu_slots = [None; MAX_CPUS];
    ...
    pub fn current_thread(&self, cpu: usize) -> Result<ThreadId, Error> {
    ...
    pub fn prepare_yield(&mut self, cpu: usize) -> Result<Option<SwitchPair>, Error> {
    ...
    pub fn prepare_exit(&mut self, cpu: usize) -> Result<SwitchPair, Error> {
        let cpu_slot = self.cpu_slot(cpu)?;
        let current = self.cpus[cpu_slot].current;
    ...
    pub fn statistics(&self) -> Statistics {
        ...
        for cpu in &self.cpus {
            if cpu.index < stats.per_cpu_ready.len() {
                stats.per_cpu_ready[cpu.index] = cpu.ready.len();
            }
        }
        stats
    }
    ...
    fn remove_ready(&mut self, id: ThreadId, cpu: usize, priority: u8) -> Result<(), Error> {
        let cpu_slot = self.cpu_slot(cpu)?;
        let (threads, cpus) = (&mut self.threads, &mut self.cpus);
    ...
    fn cpu_slot(&self, cpu: usize) -> Result<usize, Error> {
        self.cpu_slots
            .get(cpu)
            .copied()
            .flatten()
