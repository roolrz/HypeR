...
impl VirtualInterruptId {
    /// Builds a guest-visible interrupt ID whose constant value is checked at
    /// compile time.
    ///
    /// Use this for architecture ABI constants. Runtime and firmware values
    /// must continue to use [`Self::new`] and handle rejection explicitly.
    pub const fn constant<const ID: u32>() -> Self {
        const { assert!(ID <= MAX_VIRTUAL_INTERRUPT_ID) };
        Self(ID)
    }
