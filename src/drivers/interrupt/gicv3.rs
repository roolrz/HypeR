...
use crate::hal::interrupt::{InterruptController, InterruptId, InterruptTrigger};
...
impl<Cpu: CpuInterface, MemoryBarrier: Barrier> GicV3<Cpu, MemoryBarrier> {
    ...
    pub unsafe fn initialize(&mut self, affinity: u32) -> Result<(), Error> {
        ...
        unsafe { self.initialize_distributor()? };
    ...
    pub unsafe fn initialize_local(&self, affinity: u32) -> Result<(), Error> {
        ...
        unsafe { self.initialize_redistributor(redistributor)? };
    ...
    unsafe fn initialize_distributor(&mut self) -> Result<(), Error> {
        unsafe { write_u32(self.distributor.base, GICD_CTLR, 0) };
        self.wait_for_distributor_write()?;

        let typer = unsafe { read_u32(self.distributor.base, GICD_TYPER) };
        ...
        unsafe {
            write_u32(
                self.distributor.base,
                GICD_CTLR,
                GICD_CTLR_ARE_NONSECURE
                    | GICD_CTLR_ENABLE_GROUP1
                    | GICD_CTLR_ENABLE_GROUP1_AFFINITY,
            );
        }
        self.wait_for_distributor_write()
    }

    unsafe fn initialize_redistributor(&self, redistributor: usize) -> Result<(), Error> {
        let mut waker = unsafe { read_u32(redistributor, GICR_WAKER) };
        waker &= !GICR_WAKER_PROCESSOR_SLEEP;
        unsafe { write_u32(redistributor, GICR_WAKER, waker) };
        let mut remaining = REGISTER_POLL_LIMIT;
        while unsafe { read_u32(redistributor, GICR_WAKER) } & GICR_WAKER_CHILDREN_ASLEEP != 0 {
            if remaining == 0 {
        ...
        for region in &self.redistributors[..self.redistributor_count] {
                ...
                .is_some()
            {
                let base = region
                    ...
                    .ok_or(Error::AddressOverflow)?;
                let typer = unsafe { read_u64(base, GICR_TYPER) };
    ...
    fn wait_for_distributor_write(&self) -> Result<(), Error> {
        let mut remaining = REGISTER_POLL_LIMIT;
        while unsafe { read_u32(self.distributor.base, GICD_CTLR) } & GICD_CTLR_RWP != 0 {
            if remaining == 0 {
                return Err(Error::RegisterTimeout);
            }
            remaining -= 1;
    ...
    fn wait_for_redistributor_write(&self, redistributor: usize) -> Result<(), Error> {
        let mut remaining = REGISTER_POLL_LIMIT;
        while unsafe { read_u32(redistributor, GICR_CTLR) } & GICR_CTLR_RWP != 0 {
            if remaining == 0 {
                return Err(Error::RegisterTimeout);
            }
            remaining -= 1;
...
impl<Cpu: CpuInterface, MemoryBarrier: Barrier> InterruptController for GicV3<Cpu, MemoryBarrier> {
    ...
    fn enable(&mut self, interrupt: InterruptId) -> Result<(), Self::Error> {
        ...
        unsafe { write_u32(base, GICD_ISENABLER + offset, 1 << (local_id % 32)) };
        self.wait_for_write(redistributor)?;
        MemoryBarrier::data_synchronization(BarrierDomain::FullSystem, BarrierAccess::All);
        Ok(())
    }
    ...
    fn disable(&mut self, interrupt: InterruptId) -> Result<(), Self::Error> {
        ...
        let offset = (local_id / 32) as usize * 4;
        unsafe { write_u32(base, GICD_ICENABLER + offset, 1 << (local_id % 32)) };
        self.wait_for_write(redistributor)?;
        MemoryBarrier::data_synchronization(BarrierDomain::FullSystem, BarrierAccess::All);
        Ok(())
    }
...
unsafe fn write_u64(base: usize, offset: usize, value: u64) {
