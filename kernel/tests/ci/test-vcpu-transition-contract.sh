...
mkdir -p "$fixture/src/kernel/vm/vcpu"
...
write_valid_fixture() {
    printf '%s\n' \
        ...
        '        match rollback { Ok(()) => { restore_reconcile_if_claimed(execution, reconcile_claimed); crate::kernel::crash::fatal(format_args!("timer {timer:?}")) }, Err(hardware) => fatal_ambiguous_hardware("rollback failed", hardware) };' \
        '    }' \
        ...
        '                Ok(()) => { release_execution_or_fail(execution, execution_claim); Err(publication) }' \
        '                Err(timer) => { restore_reconcile_if_claimed(execution, consumed_reconcile || final_reconcile); crate::kernel::crash::fatal(format_args!("timer {timer:?}")) }' \
        '            },' \
        ...
        '    if let Err(error) = super::timer::set_host_timer_enabled(true) { crate::kernel::crash::fatal(format_args!("timer {error:?}")); }' \
        '    release_execution_or_fail(execution, claim);' \
        '}' \
        'unsafe fn deactivate_stopped(execution: &mut VcpuExecution) {' \
        '    super::active_vcpu::clear(execution);' \
        '}' >"$fixture/src/kernel/vm/vcpu/transition.rs"
}
...
mutate() {
    ...
    expression=$2
    write_valid_fixture
    sed "$expression" "$fixture/src/kernel/vm/vcpu/transition.rs" >"$fixture/mutated"
    mv "$fixture/mutated" "$fixture/src/kernel/vm/vcpu/transition.rs"
...
mutate 'timer-programming failure must not continue past rollback' \
    '13s/match rollback/return Err(timer); \/\/ match rollback/'
...
mutate 'normal teardown must release VM execution' \
    '33s/release_execution_or_fail/retain_execution_forever/'
mutate 'normal teardown must restore the host timer last' \
    '32s/set_host_timer_enabled(true)/leave_host_timer_masked()/'

write_valid_fixture
sed '4s/activate_hardware/activate_later/' \
    "$fixture/src/kernel/vm/vcpu/transition.rs" >"$fixture/mutated"
printf '%s\n' \
