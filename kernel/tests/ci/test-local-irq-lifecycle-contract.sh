...
copy_sources() {
    ...
    mkdir -p "$fixture/src/kernel/irq" "$fixture/src/kernel/device" \
        "$fixture/src/kernel/vm" "$fixture/hal/src/arch/aarch64" \
        "$fixture/hal/src/arch/riscv64" "$fixture/hal/src/arch/x86_64"
    ...
    cp "$root/hal/src/arch/aarch64/interrupts.rs" "$fixture/hal/src/arch/aarch64/interrupts.rs"
    cp "$root/hal/src/arch/aarch64/smp.rs" "$fixture/hal/src/arch/aarch64/smp.rs"
    cp "$root/hal/src/arch/riscv64/interrupts.rs" "$fixture/hal/src/arch/riscv64/interrupts.rs"
    cp "$root/hal/src/arch/riscv64/mod.rs" "$fixture/hal/src/arch/riscv64/mod.rs"
    cp "$root/hal/src/arch/x86_64/interrupts.rs" "$fixture/hal/src/arch/x86_64/interrupts.rs"
...
mutate 'AArch64 masked RPC progress mutation was accepted' hal/src/arch/aarch64/interrupts.rs \
    's/irq::service_kernel_rpc()/core::hint::spin_loop()/'
mutate 'RISC-V masked RPC progress mutation was accepted' hal/src/arch/riscv64/interrupts.rs \
    's/irq::service_kernel_rpc()/core::hint::spin_loop()/'
...
mutate 'RISC-V boot-hart SSIE arming was omitted' hal/src/arch/riscv64/mod.rs \
    's/interrupts::enable_software_interrupt_source()/interrupts::leave_software_interrupt_masked()/'
mutate 'RISC-V masked idle WFI was replaced' hal/src/arch/riscv64/mod.rs \
    's/asm!("wfi", options(nostack))/asm!("nop", options(nostack))/'
mutate 'RISC-V masked idle wait opened SIE before WFI' hal/src/arch/riscv64/mod.rs \
    's/asm!("wfi", options(nostack))/asm!("csrsi sstatus, 2", "wfi", options(nostack))/'
mutate 'AArch64 unreachable secondary route was admitted' hal/src/arch/aarch64/smp.rs \
...
mutate 'AArch64 Kernel RPC SGI gap mutation was accepted' hal/src/arch/aarch64/registers.rs \
    's/GIC_KERNEL_RPC_SGI = 8;/GIC_KERNEL_RPC_SGI = 13;/'
mutate 'AArch64 reschedule SGI mutation was accepted' hal/src/arch/aarch64/registers.rs \
