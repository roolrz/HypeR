...
copy_sources() {
    ...
    mkdir -p "$fixture/src/mm" "$fixture/src/kernel/vm/vcpu" \
        "$fixture/src/kernel/vm/registry" "$fixture/src/kernel/vm/memory" \
    ...
    cp "$root/src/kernel/vm/memory/access.rs" "$fixture/src/kernel/vm/memory/access.rs"
    cp "$root/src/kernel/vm/memory/residency.rs" \
        "$fixture/src/kernel/vm/memory/residency.rs"
...
mutate 'migratable payload must not regain an unsafe Send escape hatch' \
    src/kernel/vm/vcpu/execution.rs 'pub(crate) struct VcpuExecution {' \
...
mutate 'CPU-local ownership must retain the exact VM execution claim' \
    ...
    'claim: Option<usize>'
mutate 'registry release must reject a still-armed residency' \
...
mv "$fixture/mutated" "$fixture/src/kernel/vm/memory/residency.rs"
if check >/dev/null 2>&1; then
