...
copy_sources() {
    rm -rf "$fixture/src"
    mkdir -p "$fixture/hal/src/arch/aarch64" "$fixture/hal/src/arch/riscv64" \
        "$fixture/hal/src/arch/x86_64" "$fixture/src/kernel/boot" \
    ...
    cp "$root/hal/src/arch/aarch64/smp.rs" "$fixture/hal/src/arch/aarch64/smp.rs"
    cp "$root/hal/src/arch/riscv64/smp.rs" "$fixture/hal/src/arch/riscv64/smp.rs"
    for source_file in tlb.rs exception.rs vmx.rs interrupts.rs platform.rs smp.rs interrupt_controller.rs; do
        cp "$root/hal/src/arch/x86_64/$source_file" "$fixture/hal/src/arch/x86_64/$source_file"
    done
    ...
    cp "$root/hal/src/arch/x86_64/memory.rs" "$fixture/hal/src/arch/x86_64/memory.rs"
...
mutate 'TLB request Release mutation was accepted' hal/src/arch/x86_64/tlb.rs \
...
mutate 'TLB acknowledgement Acquire mutation was accepted' hal/src/arch/x86_64/tlb.rs \
    's/ACKNOWLEDGED_GENERATION\[cpu\].load(Ordering::Acquire)/ACKNOWLEDGED_GENERATION[cpu].load(Ordering::Relaxed)/'
mutate 'TLB acknowledgement Release mutation was accepted' hal/src/arch/x86_64/tlb.rs \
...
mutate 'CPU admission Acquire mutation was accepted' hal/src/arch/x86_64/tlb.rs \
...
mutate 'masked service Acquire mutation was accepted' hal/src/arch/x86_64/tlb.rs \
    '/fn service_pending/,/^}/s/REQUESTED_GENERATION.load(Ordering::Acquire)/REQUESTED_GENERATION.load(Ordering::Relaxed)/'
mutate 'masked service duplicate-generation check mutation was accepted' hal/src/arch/x86_64/tlb.rs \
...
mutate 'TLB polling mutation was accepted' hal/src/arch/x86_64/tlb.rs \
    's/crate::arch::irq::service_kernel_rpc()/core::hint::spin_loop()/'
mutate 'shared reason mutation was accepted' hal/src/arch/x86_64/tlb.rs \
...
mutate 'IDT interception mutation was accepted' hal/src/arch/x86_64/exception.rs \
    's/irq::service_kernel_rpc_interrupt/irq::service_rpc_later/'
mutate 'VMX interception mutation was accepted' hal/src/arch/x86_64/vmx.rs \
    's/irq::service_kernel_rpc_interrupt/irq::service_rpc_later/'
mutate 'masked progress mutation was accepted' hal/src/arch/x86_64/interrupts.rs \
    's/irq::service_kernel_rpc()/core::hint::spin_loop()/'
mutate 'AArch64 reason Release mutation was accepted' hal/src/arch/aarch64/smp.rs \
    's/fetch_or(reasons, Ordering::Release)/fetch_or(reasons, Ordering::Relaxed)/'
mutate 'RISC-V reason Acquire mutation was accepted' hal/src/arch/riscv64/smp.rs \
    's/swap(0, Ordering::Acquire)/swap(0, Ordering::Relaxed)/'
mutate 'x86 reason Release mutation was accepted' hal/src/arch/x86_64/smp.rs \
    's/fetch_or(reasons, Ordering::Release)/fetch_or(reasons, Ordering::Relaxed)/'
mutate 'CPU admission/shootdown synchronization mutation was accepted' hal/src/arch/x86_64/smp.rs \
    's/synchronize_online_cpu/synchronize_online_cpu_later/'
mutate 'online snapshot Acquire mutation was accepted' hal/src/arch/x86_64/smp.rs \
...
mutate 'live unmap without all-CPU shootdown was accepted' hal/src/arch/x86_64/memory.rs \
    '/pub unsafe fn unmap_stack/,/^    }/s/super::tlb::flush_all_online()/flush_local()/'
mutate 'x2APIC publication fence mutation was accepted' hal/src/arch/x86_64/interrupt_controller.rs \
    '/            "mfence",/d'
mutate 'x2APIC compiler-memory mutation was accepted' hal/src/arch/x86_64/interrupt_controller.rs \
    '/^pub fn send_fixed_ipi(/,/^}/s/options(nostack)/options(nomem, nostack)/'
mutate 'Kernel RPC bypassed one-shot publication' hal/src/arch/irq.rs \
...
awk '
    ...
    { print }
' "$fixture/hal/src/arch/x86_64/tlb.rs" >"$fixture/mutated"
mv "$fixture/mutated" "$fixture/hal/src/arch/x86_64/tlb.rs"
if check >/dev/null 2>&1; then
