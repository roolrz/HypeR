...
fn validate_cpu_exception_stacks() -> Result<(), Error> {
    let (irq, emergency) = stack::cpu_stack_statistics(crate::arch::current_cpu_index())
        .ok_or(Error::StackUsageMissing)?;
...
fn validate_irq_stack_switch() -> Result<(), Error> {
    ...
    let (irq, _) = stack::cpu_stack_statistics(crate::arch::current_cpu_index())
        .ok_or(Error::StackUsageMissing)?;
