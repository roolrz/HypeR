...
mod virtual_legacy_pc {
    use hyper::vm::device::x86_legacy::{InterruptSource, LegacyPcDevices};
    ...
    fn routes_uart_transmit_interrupts_through_pic_irq_four() {
        ...
        let _ = super::require_ok(devices.access(0x21, 1, true, 0xef));
        let _ = super::require_ok(devices.access(0x3f9, 1, true, 1 << 1));
        let output = super::require_ok(devices.access(0x3f8, 1, true, u32::from(b'H')));
        assert_eq!(output.transmitted, Some(b'H'));

        let interrupt = super::require_some(devices.pending_interrupt(false));
        assert_eq!(interrupt.vector, 0x24);
        assert_eq!(interrupt.source, InterruptSource::Com1);
        let identification = super::require_ok(devices.access(0x3fa, 1, false, 0));
...
mod x86_virtual_cpu_contract {
    ...
    fn decodes_io_and_ept_exit_qualifications() {
        let io = super::require_some(IoExit::decode((u64::from(0x3f8_u16) << 16) | (1 << 3)));
...
mod x86_svm_contract {
    ...
    fn decodes_io_and_nested_page_fault_exits() {
        ...
        let io = super::require_some(IoExit::decode(info));
        assert_eq!(io.port, 0x3f8);
        assert_eq!(io.size, 1);
        assert_eq!(io.direction, IoDirection::Input);
        assert!(!io.string);
