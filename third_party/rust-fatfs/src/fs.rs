...
impl<IO: Read + Write + Seek, TP, OCC> FileSystem<IO, TP, OCC> {
    ...
    /// Synchronizes filesystem metadata and flushes the underlying storage.
    ///
    /// Callers must first drop all open files and directories: their editors
    /// are not owned by the filesystem. The exclusive borrow enforces this
    /// lifetime boundary.
    /// Unlike unmounting, this preserves the mounted filesystem and its caches.
    ///
    /// # Errors
    ///
    /// Returns an error if metadata writes or the storage flush fail. A failed
