...
// HypeR: mutation owns a located SFN and its byte range, not the long name
// already consumed by matching. Keep this private and independent of the public
// iterator's owned DirEntry representation.
struct LocatedEntry<'a, IO: ReadWriteSeek, TP, OCC> {
...
impl<'a, IO: ReadWriteSeek, TP, OCC: OemCpConverter> LocatedEntry<'a, IO, TP, OCC> {
    fn from_entry(entry: &DirEntry<'a, IO, TP, OCC>) -> Self {
        Self {
    ...
    fn to_file(&self) -> File<'a, IO, TP, OCC> {
        assert!(!self.is_dir(), "Not a file entry");
        File::new(self.first_cluster(), Some(self.editor()), self.fs)
    }

    fn to_dir(&self) -> Dir<'a, IO, TP, OCC> {
        assert!(self.is_dir(), "Not a directory entry");
        match self.first_cluster() {
            Some(cluster) => Dir::new(
                DirRawStream::File(File::new(Some(cluster), Some(self.editor()), self.fs)),
                self.fs,
            ),
...
enum DirEntryOrShortName<'a, IO: ReadWriteSeek, TP, OCC> {
    DirEntry(LocatedEntry<'a, IO, TP, OCC>),
...
impl<'a, IO: ReadWriteSeek, TP: TimeProvider, OCC: OemCpConverter> Dir<'a, IO, TP, OCC> {
    // HypeR: path depth must not consume kernel stack. Each completed step
    // releases its parent stream before continuing to the next component.
    fn resolve_parent<'p>(&self, path: &'p str) -> Result<(Self, &'p str), Error<IO::Error>> {
        let mut directory = Dir::new(self.stream.clone(), self.fs);
        ...
        loop {
            ...
            let child = directory.find_directory(name)?;
            directory = child;
            remaining = rest;
        }
    }

    // HypeR: directory descent needs no returned long-name buffer. Returning
    // the small directory stream avoids retaining full DirEntry temporaries in
    // every caller while a blocking metadata read is in flight.
    #[inline(never)]
    fn find_directory(&self, name: &str) -> Result<Self, Error<IO::Error>> {
        let mut iterator = self.iter();
        while let Some(found) = iterator.visit_next(|entry| {
    ...
    ) -> Result<LocatedEntry<'a, IO, TP, OCC>, Error<IO::Error>> {
        let mut iterator = self.iter();
        while let Some(found) = iterator.visit_next(|entry| {
            if entry.eq_name(name) {
                return Some(if is_dir.is_some() && Some(entry.is_dir()) != is_dir {
                    Err(Error::InvalidInput)
                } else {
                    Ok(LocatedEntry::from_entry(entry))
                });
            }
            if let Some(ref mut generator) = short_name_gen {
                generator.add_existing(entry.raw_short_name());
            }
            None
        })? {
            if let Some(found) = found {
    ...
    pub fn open_file(&self, path: &str) -> Result<File<'a, IO, TP, OCC>, Error<IO::Error>> {
        ...
        let (parent, name) = self.resolve_parent(path)?;
        parent.find_file(name)
    }

    // HypeR: opening data needs only the small file cursor. Keep the long-name
    // entry in this lookup frame, not in every caller's return temporary.
    #[inline(never)]
    fn find_file(&self, name: &str) -> Result<File<'a, IO, TP, OCC>, Error<IO::Error>> {
        let mut iterator = self.iter();
        while let Some(found) = iterator.visit_next(|entry| {
            if !entry.eq_name(name) {
                return None;
    ...
    pub fn create_file(&self, path: &str) -> Result<File<'a, IO, TP, OCC>, Error<IO::Error>> {
        ...
        let r = parent.check_for_existence(name, Some(false))?;
        match r {
            ...
            DirEntryOrShortName::ShortName(short_name) => {
                ...
                Ok(parent.write_entry(name, sfn_entry)?.to_file())
            }
    ...
    pub fn create_dir(&self, path: &str) -> Result<Self, Error<IO::Error>> {
        ...
        let (parent, name) = self.resolve_parent(path)?;
        // this is final filename in the path
        let r = parent.check_for_existence(name, Some(true))?;
        match r {
            // directory does not exist - create it
            DirEntryOrShortName::ShortName(short_name) => {
                // alloc cluster for directory data
                let cluster = parent.fs.alloc_cluster(None, true)?;
                // create entry in parent directory
                let sfn_entry =
                    parent.create_sfn_entry(short_name, FileAttributes::DIRECTORY, Some(cluster));
                let entry = match parent.write_entry(name, sfn_entry) {
                    Ok(entry) => entry,
                    Err(Error::NotEnoughSpace) => {
                        // HypeR: no short entry was published. A full parent
                        // may consume the last available cluster for the child
                        // and then fail to extend itself. Return the unlinked
                        // allocation; never attempt this after an I/O failure,
                        // where publication may have reached the device.
                        parent.fs.free_cluster_chain(cluster)?;
                ...
                let dotdot_cluster = if parent.stream.is_root_dir() {
                    None
                } else {
                    parent.stream.first_cluster()
                };
                let sfn_entry =
                    parent.create_sfn_entry(dotdot_sfn, FileAttributes::DIRECTORY, dotdot_cluster);
                dir.write_entry("..", sfn_entry)?;
                Ok(dir)
            }
            // directory already exists - return it
    ...
    fn is_empty(&self) -> Result<bool, Error<IO::Error>> {
        let mut iterator = self.iter();
        while let Some(special) = iterator.visit_next(|entry| {
    ...
    pub fn rename(
        ...
        let (source, source_name) = self.resolve_parent(src_path)?;
        let (destination, destination_name) = dst_dir.resolve_parent(dst_path)?;
        source.rename_internal(source_name, &destination, destination_name)
    }
    ...
    fn rename_internal(
        ...
        let short_name = match r {
            ...
            DirEntryOrShortName::DirEntry(ref dst_e) => {
                ...
                if e.entry_pos == dst_e.entry_pos {
                    // nothing to do
                    return Ok(());
                }
        ...
        // HypeR: validate the moved directory's parent entry before publishing
        // another name. Directory streams may flush metadata on drop, so all
        // streams referencing the old entry must be gone before it is deleted.
        let parent_update =
            if e.is_dir() && self.stream.first_cluster() != dst_dir.stream.first_cluster() {
                let child = e.to_dir();
                let dotdot = child.find_entry("..", Some(true), None)?;
                let mut data = dotdot.data.clone();
                let parent_cluster = if dst_dir.stream.is_root_dir() {
                    None
                } else {
                    dst_dir.stream.first_cluster()
                };
                data.set_first_cluster(parent_cluster, self.fs.fat_type());
                let mut stream = child.stream.clone();
                stream.seek(SeekFrom::Start(
                    dotdot.offset_range.1 - u64::from(DIR_ENTRY_SIZE),
                ))?;
                Some((stream, data))
            } else {
                None
            };
        // HypeR: ENOSPC while extending the destination must leave the source
        // reachable. An actual I/O failure can still interrupt this unjournaled
        // transaction; callers must stop using the volume after such a failure.
        let sfn_entry = e.data.renamed(short_name);
        dst_dir.write_entry(dst_name, sfn_entry)?;
        if let Some((mut stream, data)) = parent_update {
            data.serialize(&mut stream)?;
            stream.flush()?;
        }
        // free long and short name entries
...
impl<'a, IO: ReadWriteSeek, TP: TimeProvider, OCC> DirIter<'a, IO, TP, OCC> {
    /// Inspects one borrowed entry without returning its owned long-name buffer.
    ///
    /// Like `next`, an I/O error terminates the iterator. The visitor runs once
    /// for a complete entry and cannot retain a reference to its scratch.
    /// HypeR uses this for metadata and cursor lookup on bounded kernel stacks.
    #[inline(never)]
    pub fn visit_next<R>(
        &mut self,
        visitor: impl FnOnce(&DirEntry<'a, IO, TP, OCC>) -> R,
    ) -> Result<Option<R>, Error<IO::Error>> {
        if self.err {
            return Ok(None);
        }
        let mut entry = self.entry_scratch();
        match self.read_dir_entry(&mut entry) {
            Ok(true) => Ok(Some(visitor(&entry))),
            Ok(false) => Ok(None),
            Err(error) => {
                self.err = true;
                Err(error)
    ...
    // Initialize caller-owned scratch once. Incomplete entries are never
    // exposed to the visitor or public Iterator.
    #[inline(always)]
    fn entry_scratch(&self) -> DirEntry<'a, IO, TP, OCC> {
        DirEntry {
            data: DirFileEntryData::new([0; SFN_SIZE], FileAttributes::empty()),
    ...
    fn read_dir_entry(
        &mut self,
        output: &mut DirEntry<'a, IO, TP, OCC>,
    ) -> Result<bool, Error<IO::Error>> {
        trace!("DirIter::read_dir_entry");
        #[cfg(feature = "lfn")]
        let mut lfn_builder = LongNameBuilder::new(&mut output.lfn_utf16);
        #[cfg(not(feature = "lfn"))]
        ...
        loop {
            ...
            match raw_entry {
                DirEntryData::File(data) => {
                    ...
                    output.entry_pos = abs_pos;
                    output.offset_range = (begin_offset, offset);
                    return Ok(true);
                }
...
#[cfg(feature = "lfn")]
impl<'a> LongNameBuilder<'a> {
    fn new(buf: &'a mut LfnBuffer) -> Self {
        Self {
            buf,
            chksum: 0,
            index: 0,
    ...
    fn truncate(&mut self) {
        ...
        let ucs2_units = self.buf.as_ucs2_units();
...
impl ShortNameGenerator {
    fn new(name: &str) -> Self {
        ...
        // HypeR: index 1 is not a UTF-8 boundary for a non-ASCII first
        // character. Exclude a leading dot after finding its byte position.
