...
static int write_header(uint32_t inode, uint32_t mode, uint32_t links, uint32_t size,
			const char *path)
{
	...
	const int length = snprintf(
		...
		"070701%08" PRIx32 "%08" PRIx32 "%08" PRIx32 "%08" PRIx32 "%08" PRIx32 "%08" PRIx32
		"%08" PRIx32 "%08" PRIx32 "%08" PRIx32 "%08" PRIx32 "%08" PRIx32 "%08zx%08" PRIx32,
		inode, mode, (uint32_t)0, (uint32_t)0, links, (uint32_t)0, size, (uint32_t)0,
		(uint32_t)0, (uint32_t)0, (uint32_t)0, name_size, (uint32_t)0);
...
static int write_empty_entry(uint32_t inode, uint32_t mode, uint32_t links, const char *path)
{
...
static int write_file(uint32_t inode, uint32_t permissions, const char *path, const char *source)
{
	...
	if (write_header(inode, REGULAR_MODE | permissions, 1, (uint32_t)metadata.st_size, path) !=
	    EXIT_SUCCESS) {
		fclose(input);
		return EXIT_FAILURE;
	}
...
int main(int argc, char **argv)
{
	...
	for (int index = 1; index < argc; index += 3) {
		...
		if (write_file(inode, permissions, argv[index + 1], argv[index + 2]) !=
		    EXIT_SUCCESS) {
			return EXIT_FAILURE;
		}
		++inode;
	}
	if (write_empty_entry(inode, 0, 1, "TRAILER!!!") != EXIT_SUCCESS || fflush(stdout) != 0) {
		return fail("standard output");
	}
	return EXIT_SUCCESS;
}
